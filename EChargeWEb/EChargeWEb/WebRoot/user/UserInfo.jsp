...
<ul> 
  <li><span>账户名：</span>${userInfo.u_ID}</li> 
  <li><span>手机号：</span>${userInfo.u_Tel }</li> 
  <li><span>真实姓名：</span>${userInfo.u_Name }</li> 
  <li><span>身份证件类型：</span><S:forEach var="cdtList" items="${CredentialtypeList }">
								<S:if test="${userInfo.CT_ID.equals(cdtList.CT_ID) }">
									${cdtList.CT_Name }
								</S:if>
								</S:forEach></li> 
  <li><span>证件号：</span>${userInfo.u_CredentialNum }</li> 
  <li><span>邮箱：</span>${userInfo.u_Email }</li> 
  <li><span>注册时间：</span>${userInfo.u_Time }</li>
  <li><span>账户余额：</span>${userInfo.r_Account }</li>
</ul> 
<p><input type="submit" value="修改资料" /> </p> 
</div>
</form>
